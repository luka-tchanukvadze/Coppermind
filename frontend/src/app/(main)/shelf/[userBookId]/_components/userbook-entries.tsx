...
interface UserBookEntriesProps {
  userBookId: string;
  entries: CustomData[];
}

export function UserBookEntries({ userBookId, entries }: UserBookEntriesProps) {
  return (
    <section>
      <PageHeader
        title="Entries"
        subtitle="Your thoughts, kept beside the book. Private ones stay yours."
        actions={<NewEntryDialog userBookId={userBookId} />}
        className="pb-4"
      />
      ...
      {entries.length === 0 ? (
        <div className="rounded-md border border-dashed py-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted">
            Start a note, pin a quote, draft a question.
          </p>
        </div>
      ) : (
